...
def main():
    ...
    if not args.display_all:
        ...
        p = Popen(egrep_cmd, stdin=PIPE, stdout=PIPE, stderr=STDOUT, env=os.environ.copy(), text=True)
    ...
    for i, srch in enumerate(args.searchterms):
        ...
        p = Popen(egrep_cmd, stdin=PIPE, stdout=PIPE, stderr=STDOUT, env=env, text=True)
        op, _ = p.communicate(input=op)
